...
def train_single_scale(netD,netG,reals,Gs,Zs,in_s,NoiseAmp,opt,last_scale,train_mode,train_rec_mode,first_scales_early_stop_type,
                       upsample_type,rec_upsample_type,centers=None):
    t0 = time.time()
    real = reals[len(Gs)]
    if len(Gs) > 0:
